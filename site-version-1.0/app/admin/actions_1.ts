...
export async function saveSiteSettingsAction(formData: FormData) {
  ...
  if (!current) {
    await prisma.siteSettings.create({ data: parsed });
  } else {
...
export async function saveNewsPostAction(formData: FormData) {
  ...
  const imageUrl = upload || parsed.imageUrl || existing?.imageUrl || null;
...
export async function saveTeacherAction(formData: FormData) {
  ...
  const photoUrl = upload || parsed.photoUrl || existing?.photoUrl || null;
