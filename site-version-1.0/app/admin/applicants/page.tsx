...
export default async function AdminApplicantsPage() {
  ...
  return (
    <AdminShell
      title="Абитуриентам"
      description="Настройте плашку на главной, материалы с текстом, картинками, файлами и видео, а также блок FAQ."
    >
      <div className="grid gap-8">
        ...
        <section className="grid gap-4">
          <h2 className="text-2xl font-semibold text-white">Новый FAQ</h2>
          <ApplicantFaqForm />
        </section>

        <section className="grid gap-4">
          <h2 className="text-2xl font-semibold text-white">Вопросы и ответы</h2>
          {faqs.length ? (
            faqs.map((faq) => <ApplicantFaqForm key={faq.id} faq={faq} />)
          ) : (
            <div className="rounded-[2rem] border border-dashed border-white/10 bg-white/5 p-8 text-white/65">
